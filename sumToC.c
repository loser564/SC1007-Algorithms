...
void sumToC(LinkedList* ll, int C, ArrayList* al){
    ...
    // If the sum is greater than C, stop searching
    if(ll->sum > C){
        return;
    }
    // If the sum is equal to C, add the current sequence to ArrayList
    else if(ll->sum == C){
    ...
    // Otherwise, try adding a new number to the sequence
    else{
        for(int i=1; i<C; i++){
			if(used[i] == 0){
